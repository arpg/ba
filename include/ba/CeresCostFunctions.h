                                    ...
                                    const Scalar dt)
{
    ...
    memcpy(y.t_wp.so3().data(),
           (rv2_v1.unit_quaternion()*
            pose.t_wp.so3().unit_quaternion()).coeffs().data(),sizeof(T)*4);
...
static Eigen::Matrix<T,9,1> GetPoseDerivativeJet(
    ...
    const Eigen::Matrix<T,3,1>& t_w,
    const ImuMeasurementT<Scalar>& z_tart,
    ...
    const Scalar dt)
{
    ...
    Eigen::Matrix<Scalar,3,1> zg = z_tart.w_i*alpha + z_end.w_i*(1.0-alpha);
    Eigen::Matrix<Scalar,3,1> za = z_tart.w_a*alpha + z_end.w_a*(1.0-alpha);
    ...
    // w (angular rates)
    deriv.template segment<3>(3) =
        pose.t_wp.so3().Adj()*(zg.template cast<T>()+bg);
    // a (acceleration)
    deriv.template segment<3>(6) =
        pose.t_wp.so3()*(za.template cast<T>()+ba) - t_w;
    ...
    Eigen::Matrix<Scalar,10,10>* dy_dy0 = 0)
{
    ...
    const Eigen::Matrix<T,9,1> k1 =
        GetPoseDerivativeJet<T,Scalar>(pose,g,z_start,z_end,bg,ba,0);
    const ImuPoseT<T> y1 = IntegratePoseJet<T,Scalar>(pose,k1,dt*0.5);
    const Eigen::Matrix<T,9,1> k2 =
        GetPoseDerivativeJet<T,Scalar>(y1,g,z_start,z_end,bg,ba,dt/2);
    const ImuPoseT<T> y2 = IntegratePoseJet<T,Scalar>(pose,k2,dt*0.5);
    const Eigen::Matrix<T,9,1> k3 =
        GetPoseDerivativeJet<T,Scalar>(y2,g,z_start,z_end,bg,ba,dt/2);
    const ImuPoseT<T> y3 = IntegratePoseJet<T,Scalar>(pose,k3,dt);
    const Eigen::Matrix<T,9,1> k4 =
        GetPoseDerivativeJet<T,Scalar>(y3,g,z_start,z_end,bg,ba,dt);
...
{
    ImuPoseT<T> imu_pose(pose.t_wp, pose.v_w,
                         Eigen::Matrix<T,3,1>::Zero(), pose.time);
    ...
    for (const ImuMeasurementT<Scalar>& meas : measurements) {
        if (prev_meas != 0) {
            imu_pose =
                IntegrateImuJet<T,Scalar>(imu_pose,*prev_meas,meas,bg,ba,g);
            poses.push_back(imu_pose);
        }
        prev_meas = &meas;
    }
...
struct GlobalPoseCostFunction
{
    ...
    template <typename T>
    bool operator()(const T* const t_t_ic,
                    const T* const t_t_wi, T* residuals) const
    {
       ...
       pose_residuals =
           (t_wi* t_ic *
            t_wc.inverse().template cast<T>()).log() *
           (T)weight;
...
struct FullImuCostFunction
{
        ...
        weight(w)
  {
                    ...
                    const T* const _tba, T* residuals) const
    {
        ...
        ImuPoseT<T> end_pose =
            IntegrateResidualJet<T,Scalar>(start_pose,measurements,bg,
                                           ba,g_vector,vPoses);
...
struct ImuReprojectionCostFunctor
{
  ...
  template<typename T>
  bool operator()(
      const T* const _t_wk, const T* const _r_ck,
      ...
      ) const
  {
    ...
    const Eigen::Matrix<T,2,1> z =
        ProjModel::template Project<T>(p_cv, cam_params);
    r = z - p_c.cast<T>();
