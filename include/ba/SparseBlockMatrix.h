...
#include "BlockAmbiVector.h"
#include "BlockCompressedStorage.h"
...
/** \ingroup SparseCore_Module
  ...
  * \ref TopicCustomizingEigen by defining the preprocessor symbol \c EIGEN_SparseBlockMatrix_PLUGIN.
  */
template<typename _Scalar, int _Flags = 0, typename _Index = int>  class SparseBlockMatrix;

namespace internal {
template<typename _Scalar, int _Options, typename _Index>
struct traits<SparseBlockMatrix<_Scalar, _Options, _Index> >
{
...
struct traits<Diagonal<const SparseBlockMatrix<_Scalar, _Options, _Index>, DiagIndex> >
{
    typedef SparseBlockMatrix<_Scalar, _Options, _Index> MatrixType;
...
template<typename _Scalar, int _Options, typename _Index>
class SparseBlockMatrix
        : public SparseMatrixBase<SparseBlockMatrix<_Scalar, _Options, _Index> >
{
public:
    EIGEN_SPARSE_PUBLIC_INTERFACE(SparseBlockMatrix)
    EIGEN_SPARSE_INHERIT_ASSIGNMENT_OPERATOR(SparseBlockMatrix, +=)
    EIGEN_SPARSE_INHERIT_ASSIGNMENT_OPERATOR(SparseBlockMatrix, -=)

    typedef MappedSparseMatrix<Scalar,Flags> Map;
    using Base::IsRowMajor;
    typedef internal::BlockCompressedStorage<Scalar,Index> Storage;
    enum {
        Options = _Options
    };
...
protected:
    ...
    typedef SparseBlockMatrix<Scalar,(Flags&~RowMajorBit)|(IsRowMajor?RowMajorBit:0)> TransposedSparseBlockMatrix;
...
public:
    ...
    const Diagonal<const SparseBlockMatrix> diagonal() const { return *this; }
    ...
    inline SparseBlockMatrix(Index rows, Index cols)
    ...
    inline SparseBlockMatrix(const SparseMatrixBase<OtherDerived>& other)
        : m_outerSize(0), m_innerSize(0), m_outerIndex(0), m_innerNonZeros(0)
    {
        check_template_parameters();
        *this = other.derived();
    ...
    inline SparseBlockMatrix(const SparseBlockMatrix& other)
    ...
    template<typename OtherDerived>
    SparseBlockMatrix(const ReturnByValue<OtherDerived>& other)
        : Base(), m_outerSize(0), m_innerSize(0), m_outerIndex(0), m_innerNonZeros(0)
    {
        check_template_parameters();
    ...
    inline void swap(SparseBlockMatrix& other)
    {
        //EIGEN_DBG_SPARSE(std::cout << "SparseBlockMatrix:: swap\n");
    ...
    inline SparseBlockMatrix& operator=(const SparseBlockMatrix& other)
    {
        if (other.isRValue())
        {
    ...
    inline SparseBlockMatrix& operator=(const SparseSparseProduct<Lhs,Rhs>& product)
    { return Base::operator=(product); }

    template<typename OtherDerived>
    inline SparseBlockMatrix& operator=(const ReturnByValue<OtherDerived>& other)
    {
        initAssignment(other);
    ...
    template<typename OtherDerived>
    inline SparseBlockMatrix& operator=(const EigenBase<OtherDerived>& other)
    { return Base::operator=(other.derived()); }
#endif

    template<typename OtherDerived>
    EIGEN_DONT_INLINE SparseBlockMatrix& operator=(const SparseMatrixBase<OtherDerived>& other)
    {
        ...
        if (needToTranspose)
        {
            ...
            SparseBlockMatrix dest(other.rows(),other.cols());
    ...
    friend std::ostream & operator << (std::ostream & s, const SparseBlockMatrix& m)
    {
        EIGEN_DBG_SPARSE(
                    s << "Nonzero entries:\n";
                if(m.isCompressed())
        ...
        s << static_cast<const SparseMatrixBase<SparseBlockMatrix>&>(m);
        return s;
    }
...
#   ifdef EIGEN_SparseBlockMatrix_PLUGIN
#     include EIGEN_SparseBlockMatrix_PLUGIN
#   endif
...
class SparseBlockMatrix<Scalar,_Options,_Index>::InnerIterator
{
public:
    InnerIterator(const SparseBlockMatrix& mat, Index outer)
...
template<typename Scalar, int _Options, typename _Index>
class SparseBlockMatrix<Scalar,_Options,_Index>::ReverseInnerIterator
{
public:
    ReverseInnerIterator(const SparseBlockMatrix& mat, Index outer)
...
//template<typename InputIterator, typename SparseBlockMatrixType>
//void set_from_triplets(const InputIterator& begin, const InputIterator& end, SparseBlockMatrixType& mat, int Options = 0)
//{
//  EIGEN_UNUSED_VARIABLE(Options);
//  enum { IsRowMajor = SparseBlockMatrixType::IsRowMajor };
//  typedef typename SparseBlockMatrixType::Scalar Scalar;
//  typedef typename SparseBlockMatrixType::Index Index;
//  SparseBlockMatrix<Scalar,IsRowMajor?ColMajor:RowMajor> trMat(mat.rows(),mat.cols());
...
/** Fill the matrix \c *this with the list of \em triplets defined by the iterator range \a begin - \b.
  ...
  * The matrix \c *this must be properly resized beforehand using the SparseBlockMatrix(Index,Index) constructor,
  ...
  * \code
    ...
    SparseBlockMatrixType m(rows,cols);
...
template<typename Scalar, int _Options, typename _Index>
template<typename InputIterators>
void SparseBlockMatrix<Scalar,_Options,_Index>::setFromTriplets(const InputIterators& begin, const InputIterators& end)
{
...
/** \internal */
template<typename Scalar, int _Options, typename _Index>
void SparseBlockMatrix<Scalar,_Options,_Index>::sumupDuplicates()
{
    eigen_assert(!isCompressed());
