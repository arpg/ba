                                         ...
                                         const Sophus::SE3Group<Scalar>& b)
{
  ...
  const Scalar d1 = d_2[3], d2 = d_2[4],
               d3 = d_2[5], dx = d_2[0],
               dy = d_2[1], dz = d_2[2];
...
inline Eigen::Matrix<Scalar,3,4> dLog_dq(const Eigen::Quaternion<Scalar>& q)
{
  ...
  if ( vec_norm < 1e-9 ) {
    ...
    return (Eigen::Matrix<Scalar, 3, 4>() <<
            -4*s2*powi(x,2)+s4-s1*s2, -4*x*y*s2,
            -4*x*z*s2, x*s3,
            -4*x*y*s2, -4*s2*powi(y,2) + s4 - s1*s2,
            -4*y*z*s2, y*s3,
            -4*x*z*s2, -4*y*z*s2,
            -4*s2*powi(z,2)+s4-s1*s2, z*s3
            ).finished();
    ...
    return (Eigen::Matrix<Scalar, 3, 4>() <<
            ...
            s9, -(2*x*s2)/powi(w,2),
            s10,  s7-2*powi(y,2)*s3*s4+2*powi(y,2)*s2*s5*s6,
            s8, -(2*y*s2)/powi(w,2),
            s9,  s8,
...
///////////////////////////////////////////////////////////////////////////////
template<typename Scalar=double>
inline Eigen::Matrix<Scalar,3,4> dqx_dq(const Eigen::Quaternion<Scalar>& q,
                                        const Eigen::Matrix<Scalar,3,1>& vec)
{
  ...
  return (Eigen::Matrix<Scalar, 3, 4>() <<
          ...
          s4 + 2*q.w()*y - 4*q.x()*z, 2*q.z()*y - 2*q.w()*x - 4*q.y()*z,
          s2 + s3, s1 - 2*q.y()*x
          ).finished();
}
...
///////////////////////////////////////////////////////////////////////////////
template<typename Scalar=double>
inline Sophus::SE3Group<Scalar> exp_decoupled(const Sophus::SE3Group<Scalar>& a,
    ...
    const Sophus::SE3Group<Scalar>& b)
{
  const Eigen::Matrix<Scalar,6,1> d_2 =
      Sophus::SE3Group<Scalar>::log(a*b.inverse())/2;
  const Scalar d1 = d_2[3], d2 = d_2[4], d3 = d_2[5],
               dx = d_2[0], dy = d_2[1], dz = d_2[2];

  // this is using the 2nd order cbh expansion, to evaluate
  // (I + 0.5 [Adj*x, log(AB)])*Adj
  // refer to the thesis by Hauke Strasdat, Appendix 3.
  const Eigen::Matrix<Scalar, 6, 6> d_log ((Eigen::Matrix<Scalar, 6, 6>() <<
                                           1,  d3, -d2,   0,  dz, -dy,
                                           -d3,   1,  d1, -dz,   0,  dx,
                                           d2, -d1,   1,  dy, -dx,   0,
                                           0,   0,   0,   1,  d3, -d2,
...
inline Eigen::Matrix<Scalar, 6, 7> dLog_dSE3(Sophus::SE3Group<Scalar> t){
  ...
  const Eigen::Matrix<Scalar, 3, 3> dlog_dw((Eigen::Matrix<Scalar, 3, 3>() <<
   ...
   y*(s20 + s6*s7*s10 - 2*s3*s8*s10*wx) -
   s18*x + z*(s12 - s6*s7*wy*wz + 0.5),

   y*(s5*s7*s10 - 2*s3*s8*s10*wy) - s17*x - s15*z,
   y*(s4*s7*s10 + 2*s3*s7*wz - 2*s3*s8*s10*wz) -
